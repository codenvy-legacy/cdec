...
public class CheckoutSubscriptionCommandTest {
    ...
    public void initMocks() {
        ...
        spyCommand = spy(new CheckSubscriptionCommand());
    ...
    @Test
    public void testCheckSubscriptionReturnErrorResponse() throws Exception {
        String errorServiceResponse = "{\nstatus: \"ERROR\"}";
        doReturn(errorServiceResponse).when(mockInstallationManagerProxy).checkSubscription(anyString(), eq(userCredentialsRep));

        CommandInvoker commandInvoker = new CommandInvoker(spyCommand, commandSession);

        CommandInvoker.Result result = commandInvoker.invoke();
        String output = result.getOutputStream();
        assertTrue(output.contains(Commons.getPrettyPrintingJson(errorServiceResponse)));
    }
}
