...
public class TestNodeManager extends BaseTest {
    ...
    public void testGetAddNodeCommand() throws Exception {
        ...
        assertEquals(commands.get(12).toString(), format("{'command'='sudo puppet agent --onetime --ignorecache --no-daemonize --no-usecacheonfailure --no-splay; exit 0;', 'agent'='{'host'='127.0.0.1', 'user'='%1$s', 'identity'='[~/.ssh/id_rsa]'}'}", SYSTEM_USER_NAME));
    ...
    public void testGetRemoveNodeCommand() throws Exception {
        ...
        assertEquals(commands.get(1).toString(),
                     ...
                     "| sed 's|~n|\\n|g' > tmp.tmp " +
                     "&& sudo mv tmp.tmp /etc/puppet/manifests/nodes/multi_server/custom_configurations.pp', 'agent'='LocalAgent'}");
        assertEquals(commands.get(2).toString(), format("{'command'='sudo puppet agent --onetime --ignorecache --no-daemonize --no-usecacheonfailure --no-splay; exit 0;', 'agent'='{'host'='127.0.0.1', 'user'='%1$s', 'identity'='[~/.ssh/id_rsa]'}'}", SYSTEM_USER_NAME));
